...
from src.data import repro_load_dataset
...
class Solver(BaseSolver):
    ...
    def load_data(self):
        ...
        self.tt_set, self.feat_dim, self.vocab_size, self.tokenizer, msg = \
            repro_load_dataset(self.paras.njobs, self.paras.gpu,
                         self.paras.pin_memory, False, **self.config['data'])
        self.verbose(msg)
        print('data num:',len(self.tt_set))
    ...
    def exec(self):
        ...
        # Setup output
        self.cur_output_path = self.output_file.format('test', 'output')
        ...
        if self.greedy:
            # Greedy decode
            self.verbose(
                'Performing batch-wise greedy decoding on test set, num of batch = {}.'.format(len(self.tt_set)))
            self.verbose('Results will be stored at {}'.format(
                self.cur_output_path))
        else:
            # Additional output to store all beams
            self.cur_beam_path = self.output_file.format('test', 'beam')
            with open(self.cur_beam_path, 'w') as f:
                f.write('idx\tbeam\thyp\ttruth\n')
            self.verbose(
                'Performing instance-wise beam decoding on test set. (NOTE: use --njobs to speedup)')
            ...
            results = Parallel(n_jobs=self.paras.njobs)(
                delayed(beam_decode_func)(data) for data in tqdm(self.tt_set))
            self.verbose(
