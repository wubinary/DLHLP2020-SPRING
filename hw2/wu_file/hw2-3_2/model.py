...
class Encoder(nn.Module):
    def __init__(self, c_in=513, c_h1=256, c_h2=256, c_h3=128, ns=0.2, dp=0.5):
    ...
    def forward(self, x):
        outs = []
        for l in self.conv1s: #各個kernel_size:1~8的 1d_conv
            out = pad_layer(x, l)
            outs.append(out)
        ...
        mean = linear(out, self.linear0) #mean = F.relu(mean)#, negative_slope=self.ns)
...
class Decoder(nn.Module):
    def __init__(self, c_in=256, c_out=513, c_h=256, c_a=8, emb_size=128, ns=0.2):
    ...
    def conv_block(self, x, conv_layers, norm_layer, emb, res=True):
        ...
        out = pixel_shuffle_1d(out, upscale_factor=2) #[64,512,16]->[64,256,32]
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.encoder = Encoder()
        self.linear = nn.Linear(256*16, 128)
        
    def forward(self, x):
        b = x.shape[0]
        output, _ = self.encoder(x) #output:(b, 128, 16)
        output = output.contiguous().view(b,-1)
        output = self.linear(output) #(b, 128*16)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(1)
        output = self.main(x)
        output = output.view(b, -1, 1216)
        output = self.fc(output) #等於一堆分數
        output = torch.clamp(output, -1, 1)
        return output #(b, 128, 1)
'''   
