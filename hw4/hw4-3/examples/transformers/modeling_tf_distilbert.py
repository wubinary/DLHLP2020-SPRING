...
class TFFFN(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        assert config.activation in [
            'relu', 'gelu'], "activation ({}) must be in ['relu', 'gelu']".format(config.activation)
        self.activation = tf.keras.layers.Activation(
            gelu) if config.activation == 'gelu' else tf.keras.activations.relu
...
class TFTransformerBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):  # removed: src_enc=None, src_len=None
        ...
        sa_output = self.attention(
            [x, x, x, attn_mask, head_mask], training=training)
        if self.output_attentions:
        ...
        sa_output = self.sa_layer_norm(
            sa_output + x)          # (bs, seq_length, dim)
    ...
    def call(self, inputs, training=False):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                [hidden_state, attn_mask, head_mask[i]], training=training)
...
class TFDistilBertMainLayer(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.embeddings = TFEmbeddings(
            config, name="embeddings")   # Embeddings
    ...
    def call(self, inputs, attention_mask=None, head_mask=None, inputs_embeds=None, training=False):
        ...
        embedding_output = self.embeddings(
            input_ids, inputs_embeds=inputs_embeds)   # (bs, seq_length, dim)
...
class TFDistilBertForMaskedLM(TFDistilBertPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFDistilBertForMaskedLM, self).__init__(
...
class TFDistilBertForSequenceClassification(TFDistilBertPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFDistilBertForSequenceClassification,
    ...
    def call(self, inputs, **kwargs):
        ...
        pooled_output = self.dropout(pooled_output, training=kwargs.get(
            'training', False))         # (bs, dim)
...
class TFDistilBertForQuestionAnswering(TFDistilBertPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFDistilBertForQuestionAnswering, self).__init__(
            config, *inputs, **kwargs)
    ...
    def call(self, inputs, **kwargs):
        ...
        hidden_states = self.dropout(hidden_states, training=kwargs.get(
            'training', False))                       # (bs, max_query_len, dim)
        # (bs, max_query_len, 2)
